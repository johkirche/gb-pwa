<template>
        ...
        <CardContent>
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div v-for="file in files" :key="file.id"
                    class="border rounded-lg p-4 hover:shadow-md transition-shadow"> <!-- Image Preview -->
                    <div v-if="file.type.includes('image')" class="mb-3">
                        <div class="relative aspect-video w-full overflow-hidden rounded-md bg-gray-100 cursor-pointer group"
                            @click="openImagePreview(file)">
                            <NuxtImg :src="`${directusUrl}/assets/${file.id}?width=300&height=200&fit=cover`"
                                :alt="file.title || file.filename_download || 'Image'"
                                class="w-full h-full object-cover transition-transform group-hover:scale-105"
                                loading="lazy" @error="handleImageError" @load="handleImageLoad" />
                            <!-- Fallback for failed images -->
                            <div v-if="imageErrors.has(file.id)"
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="font-medium text-sm truncate">{{ file.title || file.filename_download }}</p>
                            <p class="text-xs text-muted-foreground">{{ file.type }}</p>
                            <p class="text-xs text-muted-foreground">{{ formatFileSize(file.filesize) }}</p>
                            <div class="flex items-center gap-2 mt-1">
                                <Button v-if="file.type.includes('image')" @click="openImagePreview(file)"
                                    variant="link">
                                    Preview
                                    <Eye class="w-3 h-3 ml-1" />
                                </button>
                                <Button variant="link" @click="downloadFile(file)" rel="noopener noreferrer"
    ...
    <Dialog v-model:open="isImagePreviewOpen">
        <DialogContent class="max-w-4xl w-full h-[95vh] p-0 bg-black border-none flex flex-col text-white">
            <DialogHeader
                class="flex-shrink-0 bg-black bg-opacity-80 backdrop-blur-sm p-4 border-b border-gray-700 rounded-t-lg">
                <DialogTitle class="text-white">
                    {{ selectedImage?.title || selectedImage?.filename_download || 'Image Preview' }}
                </DialogTitle>
                <DialogDescription class="text-gray-300">
                    {{ selectedImage?.type }} • {{ formatFileSize(selectedImage?.filesize) }}
                    <span class="text-gray-400 text-xs block mt-1">Click image for fullscreen view</span>
                </DialogDescription>
            </DialogHeader>
            <div class="flex-1 flex items-center justify-center p-4 min-h-0">
                <NuxtImg v-if="selectedImage" :src="`${directusUrl}/assets/${selectedImage.id}`"
                    ...
                    @error="handleDialogImageError" @load="handleDialogImageLoad" @click="openFullscreenImage" />
    ...
    <PanZoomImage v-model:is-open="isFullscreenOpen"
        :image-src="fullScreenImage ? `${directusUrl}/assets/${fullScreenImage.id}` : ''"
        :image-alt="fullScreenImage?.title || fullScreenImage?.filename_download || 'Image'"
        @close="handleFullscreenClose" @image-error="handleDialogImageError" @image-load="handleDialogImageLoad" />
</template>

<script setup lang="ts">
import { Files, FileText, Music, Image, File, Download, Eye } from "lucide-vue-next";
