                    ...
                    <div class="flex items-start space-x-4">
                        ...
                        <div class="flex-1 space-y-2">
                            <div class="bg-muted/50 p-4 rounded-lg">
                                <pre
                                    class="whitespace-pre-wrap font-serif text-base leading-relaxed">{{ strophe?.strophe }}</pre>
                            </div>

                            <!-- Change suggestions -->
                            <div v-if="strophe?.aenderungsvorschlag"
                                class="bg-yellow-50 border border-yellow-200 p-3 rounded-lg">
                                <p class="text-sm font-medium text-yellow-800 mb-1">Change Suggestion:</p>
                                <pre
                                    class="whitespace-pre-wrap text-sm text-yellow-700">{{ strophe.aenderungsvorschlag }}</pre>
                            </div>

                            <!-- Notes -->
                            <div v-if="strophe?.anmerkung" class="bg-blue-50 border border-blue-200 p-3 rounded-lg">
