<template>
    <Teleport to="body">
        <div v-if="isOpen" class="fixed inset-0 z-[99999] bg-black flex items-center justify-center select-none"
            @click="handleBackgroundClick">
            <!-- Close button -->
            <div class="absolute top-4 right-4 flex gap-2 z-[100000]">
                <Button class="text-white !bg-black !bg-opacity-50 hover:!bg-opacity-70 rounded-full transition-colors"
                    @click.stop="closeModal" aria-label="Close">
                    <X class="w-6 h-6" />
                </Button>
            </div>
            ...
            <div class="absolute top-4 left-4 flex flex-col gap-2 z-[100000]">
                <Button size="icon"
                    ...
                    @click.stop="zoomIn" aria-label="Zoom in">
                    <Plus class="w-6 h-6" />
                </Button>
                <Button size="icon"
                    class="text-white !bg-black !bg-opacity-50 hover:!bg-opacity-70 rounded-full transition-colors"
                    @click.stop="zoomOut" aria-label="Zoom out">
                    <Minus class="w-6 h-6" />
                </Button>
                <Button size="icon"
                    class="text-white !bg-black !bg-opacity-50 hover:!bg-opacity-70 rounded-full transition-colors"
                    @click.stop="resetZoom" aria-label="Reset zoom">
            ...
            <div ref="imageContainer"
                ...
                <NuxtImg v-if="imageSrc" ref="panZoomImage" :src="imageSrc" :alt="imageAlt"
                    :style="{ width: 'auto', height: 'auto' }" class="max-w-full max-h-full object-contain"
...
const handleImageError = (event: Event) => {
    console.error('Pan-zoom image failed to load:', (event.target as HTMLImageElement).src);
...
const initializePanzoom = () => {
    ...
    if (!panzoomInstance) {
        ...
        imageContainer.value.addEventListener('wheel', (event) => {
            if (panzoomInstance) {
