            ...
            <DialogContent class="max-w-4xl max-h-[90vh] flex flex-col">
                ...
                <div class="space-y-4 flex-1 overflow-hidden">
                    ...
                    <div class="relative">
                        ...
                        <Input v-model="searchQuery" :placeholder="t('churchService.searchSongs')" class="pl-10" />
                    </div>
                    ...
                    <ScrollArea class="h-[450px]">
                        <div class="space-y-2 pr-4">
                            <div
v-for="song in filteredSongs" :key="song.id" :class="[
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center space-x-2 mt-1">
                                            ...
                                            <Badge
v-for="category in getCategories(song).slice(0, 2)" :key="category"
                                    ...
                                    <Button
v-if="hasAudioFiles(song)" variant="ghost" size="sm"
...
const filteredSongs = computed(() => {
    ...
    return gesangbuchlieder.value.filter(song =>
...
const hasAudioFiles = (song: Gesangbuchlied): boolean => {
    return !!(
        song.melodieId?.noten?.some(note =>
...
const getAuthors = (song: Gesangbuchlied): string => {
    const textAuthors = song.textId?.autorId?.map(author =>
        `${author?.autor_id?.vorname || ''} ${author?.autor_id?.nachname || ''}`.trim()
    ).filter(Boolean) || [];

    const melodieAuthors = song.melodieId?.autorId?.map(author =>
        `${author?.autor_id?.vorname || ''} ${author?.autor_id?.nachname || ''}`.trim()
    ).filter(Boolean) || [];
...
const getCategories = (song: Gesangbuchlied): string[] => {
    return song.kategorieId?.map(kat => kat?.kategorie_id?.name).filter(Boolean) || [];
};
