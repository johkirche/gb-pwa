<template>
  <div class="max-w-6xl mx-auto p-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
      ...
      >
        <strong>Warning:</strong> Web MIDI API is not supported in this browser. Please use Chrome,
        Edge, or Opera.
      </div>

      <!-- MIDI Access Request -->
      <div
        v-if="webMidiSupported && !midiAccess"
      ...
      <div v-if="midiData && selectedOutputId" class="mb-6">
        ...
        <div v-if="isPlaying" class="mt-4">
          ...
          <p class="text-sm text-gray-600 mt-1">{{ Math.round(playbackProgress) }}% complete</p>
        </div>
      </div>
      ...
      <div v-if="midiData" class="bg-gray-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div><strong>Format:</strong> {{ midiData.format }}</div>
          <div><strong>Tracks:</strong> {{ midiData.tracks.length }}</div>
          <div><strong>Ticks per Quarter Note:</strong> {{ midiData.ticksPerQuarter }}</div>
          <div><strong>Duration:</strong> {{ formatDuration(midiData.duration) }}</div>
        </div>
      </div>
    </div>
  </div>
