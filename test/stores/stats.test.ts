...
// ---------------------------------------------------------------------------
// Regression guard for issue #25 — https://github.com/johkirche/gb-pwa/issues/25
//
// loadStats hardcoded `recentlyPlayed: 0` behind a "TODO: Get from local
// storage", on both the success and the error path, so the home-screen
// "Kürzlich gespielt" tile (StatsRow.vue) read 0 forever on every device for
// every user — a counter that looked broken rather than unbuilt.
//
// The number is derived from the played-service history, the only record of a
// play the app keeps. These tests use the real (fake-indexeddb) database,
// because that history is the thing under test: a fresh IDBFactory per test,
// since the database name is a module constant and `dbInitialized` lives on the
// store instance, so a fresh pinia re-opens against the new factory.
// ---------------------------------------------------------------------------
describe("stats store — recentlyPlayed", () => {
  beforeEach(() => {
  ...
  it("counts songs from the played-service history", async () => {
    ...
    expect(stats.stats.recentlyPlayed).toBe(3);
  });
  ...
  it("keeps reporting played songs when the network total cannot be fetched", async () => {
    ...
    expect(stats.stats.recentlyPlayed).toBe(2);
    expect(stats.stats.totalSongs).toBe(0);
  });

  it("counts a song played in two services once", async () => {
    // The tile counts songs, not performances, so a hymn sung every Sunday must
    // not inflate it.
    signIn();
    post.mockResolvedValue(ok(countPayload(42)));
    await playAndSaveService("Ostersonntag", ["s-1", "s-2"]);
    await playAndSaveService("Erntedank", ["s-1"]);
    const stats = useStatsStore();
  ...
  it("clears the counter with the rest of the stats on logout", async () => {
    ...
    const stats = useStatsStore();
    await stats.loadStats();
    expect(stats.stats.recentlyPlayed).toBe(1);
